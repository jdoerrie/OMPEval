...
class TestException : public std::exception {
 ...
 public:
  TestException(const std::string& msg) : mMg(msg) { ; }

  const char* what() const throw() override { return mMg.c_str(); }
};
...
class TestBase {
 ...
 protected:
  ...
  static void testEqual(const char* expr, const T1& actual,
                        const T2& expected) {
    if (actual != expected) {
      std::stringstream ss;
      ss << "\"" << expr << "\", expected \"" << expected << "\", was \""
         << actual << "\"";
...
// Declares a member function to be called before each test case. Must be
// followed by function body.
#define TTEST_BEFORE() virtual void ttestBefore() override

// Registers and declares a test case member function. Must be followed by
// function body.
#define TTEST_CASE(DESCRIPTION)                                          \
  int TTEST_CONCAT(TTEST_UNIQUE_TEST_ID, _init) =                        \
      (addTestCase([this] { TTEST_UNIQUE_TEST_ID(); }, DESCRIPTION), 0); \
