...
class AdaptorBase {
 public:
  ...
  static unsigned cardIdxFromCanonical(unsigned idx) {
    ...
    unsigned suit = idx & 3;
    rank = TEval::ASCENDING_RANKS ? rank : 12 - rank;
    return (TEval::RANK_MAJOR ? 4 * rank + suit : 13 * suit + rank) +
           TEval::CARD_OFFSET;
  }
};
...
class Omp : public omp::HandEvaluator, public AdaptorBase<Omp, omp::Hand> {
 public:
  void initHand(Hand& h) const { h = Hand::empty(); }

  void addCard(Hand& h, unsigned cardIdx) const { h += cardIdx; }

  unsigned evaluate(const Hand& h, unsigned c1, unsigned c2, unsigned c3,
                    unsigned c4, unsigned c5, unsigned c6, unsigned c7) const {
...
// 2+2 Evaluator
// This requires some modifications to 2+2 code so that we don't have to read
...
class Tpt : public AdaptorBase<Tpt> {
 public:
  ...
  Tpt() { static bool initVar = (generateArrays(), initVar); }

  void initHand(Hand& h) const { h = 53; }

  void addCard(Hand& h, unsigned cardIdx) const { h = HR[h + cardIdx]; }
...
class Ace : public AdaptorBase<Ace, array<Card, ACEHAND>> {
 public:
  ...
  void initHand(Hand& h) const { h = Hand{}; }

  void addCard(Hand& h, unsigned cardIdx) const {
    static uint32_t ACE_CARDS[52] = {
        0x41,       0x101,      0x401,      0x1001,    0x4001,    0x10001,
        0x40001,    0x100001,   0x400001,   0x1000001, 0x4000001, 0x10000001,
        0x40000001, 0x42,       0x102,      0x402,     0x1002,    0x4002,
        0x10002,    0x40002,    0x100002,   0x400002,  0x1000002, 0x4000002,
        0x10000002, 0x40000002, 0x44,       0x104,     0x404,     0x1004,
        0x4004,     0x10004,    0x40004,    0x100004,  0x400004,  0x1000004,
        0x4000004,  0x10000004, 0x40000004, 0x48,      0x108,     0x408,
        0x1008,     0x4008,     0x10008,    0x40008,   0x100008,  0x400008,
        0x1000008,  0x4000008,  0x10000008, 0x40000008};
    ACE_addcard(h, ACE_CARDS[cardIdx]);
  }

  unsigned evaluate(const Hand& h, unsigned c1, unsigned c2, unsigned c3,
                    unsigned c4, unsigned c5, unsigned c6, unsigned c7) const {
    Card E(Card h[]);  // Works if ACE compiled as c++, otherwise we need extern
                       // C block.
...
class Sbhs : public AdaptorBase<Sbhs, Hand_T> {
 public:
  ...
  Sbhs() { static bool initVar = (Init_Hand_Eval(), initVar); }

  void initHand(Hand& h) const { ZeroHand(h); }
...
class Benchmark {
 public:
  ...
  void run() {
    cout << endl;
    if (!is_same<TEval, Omp>::value) test(Omp());
    sequential<false>();
    random1();
    if (!is_same<Hand, nullptr_t>::value) random2();
    sequential<true>();
  }
 ...
 private:
  ...
  void sequential() {
    cout << "Sequential evaluation" << (tSingleSuit ? " (flush hands):" : ":")
         << endl;
    ...
    double t =
        1e-9 * chrono::duration_cast<chrono::nanoseconds>(t2 - t1).count();
    cout << "   " << count << " evals  " << (1e-6 * count / t) << "M/s  " << t
         << "s  " << sum << endl;
  }
  ...
  void random1() {
    ...
    for (int i = 0; i < 50; ++i) {
      for (auto& cards : table) {
        ...
        sum += mEval.evaluate(h, cards[0], cards[1], cards[2], cards[3],
  ...
  // Generate a vector of random hands. The random seed is deterministic on
  // purpose.
  ...
  // Test this evaluator against another one by comparing the order in which
  // they rank random hands.
  ...
  template <class TEval2>
  static vector<pair<size_t, unsigned>> sortHands(
      const TEval2& eval, const vector<array<uint8_t, 7>>& table) {
    ...
    for (auto& cards : table) {
      ...
      eval.initHand(h);
      for (uint8_t c : cards) eval.addCard(h, TEval2::cardIdxFromCanonical(c));
      unsigned hv = eval.evaluate(h, TEval2::cardIdxFromCanonical(cards[0]),
    ...
    sort(handValues.begin(), handValues.end(),
         [](const pair<size_t, unsigned>& lhs,
...
void benchmark() {
  // Benchmark only one at a time because there's some weird performance
  // interference.
  Benchmark<Omp>().run();
  // Benchmark<Skpe>().run();
  // Benchmark<Tpt>().run();
