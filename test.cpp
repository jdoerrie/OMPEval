...
class HandTest : public ttest::TestBase {
  ...
  TTEST_CASE("allocated objects are aligned") {
    struct HandHash {
      size_t operator()(const Hand& h) const { return h.rankKey(); }
    };
  ...
  TTEST_CASE("rankKey()") {
    ...
    TTEST_EQUAL((Hand(4) + Hand(8)).rankKey() == (Hand(12) + Hand(5)).rankKey(),
                false);
  }
...
class HandEvaluatorTest : public ttest::TestBase {
  ...
  void enumerate(unsigned cardsLeft, const Hand& h = Hand::empty(),
                 unsigned start = 0) {
  ...
  TTEST_BEFORE() { fill(begin(counts), end(counts), 0); }
  ...
  TTEST_CASE("enumerate 1 card hands") {
    ...
    enumerate(1);
    for (unsigned i = 0; i < 10; ++i) TTEST_EQUAL(counts[i], expected[i]);
  }
  ...
  TTEST_CASE("enumerate 5 cards hands") {
    uint64_t expected[10]{0,     1302540, 1098240, 123552, 54912,
                          10200, 5108,    3744,    624,    40};
    enumerate(5);
    for (unsigned i = 0; i < 10; ++i) TTEST_EQUAL(counts[i], expected[i]);
  }

  TTEST_CASE("enumerate 6 cards hands") {
    uint64_t expected[10]{0,      6612900, 9730740, 2532816, 732160,
                          361620, 205792,  165984,  14664,   1844};
    enumerate(6);
    for (unsigned i = 0; i < 10; ++i) TTEST_EQUAL(counts[i], expected[i]);
  }

  TTEST_CASE("enumerate 7 cards hands") {
    uint64_t expected[10]{0,       23294460, 58627800, 31433400, 6461620,
...
class EquityCalculatorTest : public ttest::TestBase {
  ...
  vector<TestCase> TESTDATA = [&] {
    vector<TestCase> td;
    td.emplace_back(
        TestCase{{"AA", "KK"}, "", "", {0, 50371344, 10986372, 285228}});
    td.emplace_back(TestCase{
        {"AK", "random"}, "2c3c", "", {0, 159167583, 108567320, 6233737}});
    td.emplace_back(TestCase{{"random", "AA", "33"},
                             "2c3c8h",
                             "6h",
                             {0, 808395, 1681125, 20076, 12151512, 0, 0, 0}});
    td.emplace_back(
        TestCase{{"random", "random", "AK"},
                 "4hAd3c4c7c",
                 "6h",
                 {0, 1461364, 1461364, 6386, 6760010, 42420, 42420, 108}});
    td.emplace_back(TestCase{{"3d7d", "2h9h", "2c9c"},
                             "5d5h5c",
                             "3s3c",
    ...
    td.emplace_back(TestCase{{"AA,KK", "KK,QQ", "QQ,AA"},
                             "",
                             "",
                             {0, 348272820, 119882736, 37653912, 303253020,
                              74015280, 1266624, 3904200}});
    return td;
  ...
  void enumTest(const TestCase& tc) {
    ...
    if (!eq.start(ranges2, CardRange::getCardMask(tc.board),
                  CardRange::getCardMask(tc.dead), true))
  ...
  void monteCarloTest(const TestCase& tc) {
    double hands =
        accumulate(tc.expectedResults.begin(), tc.expectedResults.end(), 0.0);
    std::vector<CardRange> ranges2(tc.ranges.begin(), tc.ranges.end());
    bool timeout = false;
    auto callback = [&](const EquityCalculator::Results& r) {
      double maxErr = 0;
      for (unsigned i = 0; i < (1u << tc.ranges.size()); ++i)
        maxErr = max(std::abs(tc.expectedResults[i] / hands -
                              (double)r.winsByPlayerMask[i] / r.hands),
                     maxErr);
      if (maxErr < 2e-4) eq.stop();
      if (r.time > 10) {
        timeout = true;
        eq.stop();
      }
    ...
    if (!eq.start(ranges2, CardRange::getCardMask(tc.board),
                  CardRange::getCardMask(tc.dead), false, 0, callback, 0.1))
      throw ttest::TestException("Invalid hand ranges!");
    eq.wait();
    if (timeout)
  ...
  TTEST_CASE("start() returns false when too many board cards") {
    TTEST_EQUAL(eq.start({"random"}, CardRange::getCardMask("2c3c4c5c6c7c")),
                false);
  }
  ...
  TTEST_CASE(
      "start() returns false when hand range is empty after card removal") {
    TTEST_EQUAL(eq.start({"random", ""}), false);
    TTEST_EQUAL(eq.start({"AA", "22"}, CardRange::getCardMask("AsAh"),
                         CardRange::getCardMask("Ac")),
                false);
  }

  TTEST_CASE(
      "start() returns false when no feasible combination of holecards") {
    TTEST_EQUAL(eq.start({"AA", "AK"}, CardRange::getCardMask("As"),
                         CardRange::getCardMask("Ah")),
                false);
  }
