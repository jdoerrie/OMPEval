...
bool EquityCalculator::start(const std::vector<CardRange>& handRanges,
                             uint64_t boardCards, uint64_t deadCards,
                             bool enumerateAll, double stdevTarget,
                             std::function<void(const Results&)> callback,
                             double updateInterval, unsigned threadCount) {
  if (handRanges.size() == 0 || handRanges.size() > MAX_PLAYERS) return false;
  if (bitCount(boardCards) > BOARD_CARDS) return false;
  ...
  std::vector<CombinedRange> combinedRanges =
      CombinedRange::joinRanges(mHandRanges, MAX_COMBINED_RANGE_SIZE);
  for (unsigned i = 0; i < combinedRanges.size(); ++i) {
    if (combinedRanges[i].combos().size() == 0) return false;
    if (!enumerateAll) combinedRanges[i].shuffle();
...
void EquityCalculator::simulateRegularMonteCarlo() {
  ...
  for (unsigned i = 0; i < mCombinedRangeCount; ++i)
    comboDists[i] = FastUniformIntDistribution<unsigned, 21>(
        0, (unsigned)mCombinedRanges[i].combos().size() - 1);
  ...
  for (;;) {
    ...
    randomizeBoard(board, remainingCards,
                   usedCardsMask | mDeadCards | mBoardCards, rng, cardDist);
...
// Monte carlo simulation using a random walk. On each iteration a random player
// is chosen and the next feasible combo is picked for that player. To prove
// that each preflop really has equal probability of being visited the preflop
// combinations can be thought of as a directed k-regular graph. The transition
// probability matrix P then has k non-zero values on each row and column, and
// all non-zero elements have value of 1/k. It is easy to see that (1,1,...,1) *
// P = (1,1,...,1), i.e. (1,1,...,1) is a stable distribution.
void EquityCalculator::simulateRandomWalkMonteCarlo() {
  ...
  FastUniformIntDistribution<unsigned, 16> cardDist(0, CARD_COUNT - 1);
  FastUniformIntDistribution<unsigned, 21> comboDists[MAX_PLAYERS];
  FastUniformIntDistribution<unsigned, 16> combinedRangeDist(
      0, mCombinedRangeCount - 1);
  for (unsigned i = 0; i < mCombinedRangeCount; ++i)
    comboDists[i] = FastUniformIntDistribution<unsigned, 21>(
        0, (unsigned)mCombinedRanges[i].combos().size() - 1);
  ...
  // Set initial state.
  if (randomizeHoleCards(usedCardsMask, comboIndexes, playerHands, rng,
                         comboDists)) {
    // Loop until stopped.
    for (;;) {
      ...
      if ((stats.evalCount & 0xfff) == 0) {
        ...
        if (mStopped) break;
        stats = BatchResults(nplayers);
        // Occasionally do a full randomization, because in some rare cases the
        // random walk might not be able to visit all preflop combinations by
        // changing just one hand at a time. This shouldn't happen if
        // MAX_COMBINED_RANGE_SIZE is big enough, but extra randomization never
        // hurts.
        if (!randomizeHoleCards(usedCardsMask, comboIndexes, playerHands, rng,
                                comboDists))
          break;
      ...
      // Choose random player and iterate to next valid combo. If current combo
      // is the only one that is valid then will loop back to itself.
      unsigned combinedRangeIdx = combinedRangeDist(rng);
      const CombinedRange& combinedRange = mCombinedRanges[combinedRangeIdx];
      unsigned comboIdx =
          comboIndexes[combinedRangeIdx];  // Caching array accessess for 3%
                                           // speedup!
      usedCardsMask -= combinedRange.combos()[comboIdx].cardMask;
      uint64_t mask = 0;
      do {
        if (comboIdx == 0) comboIdx = (unsigned)combinedRange.size();
        --comboIdx;
...
// Randomize holecards using rejection sampling. Returns false if maximum number
// of attempts was reached.
bool EquityCalculator::randomizeHoleCards(
    uint64_t& usedCardsMask, unsigned* comboIndexes, Hand* playerHands,
...
void EquityCalculator::randomizeBoard(
    Hand& board, unsigned remainingCards, uint64_t usedCardsMask, Rng& rng,
    FastUniformIntDistribution<unsigned, 16>& cardDist) {
  omp_assert(remainingCards + bitCount(usedCardsMask) <= CARD_COUNT &&
             remainingCards <= BOARD_CARDS);
...
void EquityCalculator::evaluateHands(const Hand* playerHands, unsigned nplayers,
                                     const Hand& board, BatchResults* stats,
                                     unsigned weight) {
...
void EquityCalculator::enumerate() {
  ...
  for (unsigned i = 0; i < combinedRangeCount; ++i)
    fastDividers[i] =
        libdivide::libdivide_u64_gen(mCombinedRanges[i].combos().size());
  ...
  // Disable random preflop enumeration order if postflop is too small (bad for
  // caching). It's also makes no sense if all the combos don't fit in the
  // lookup table.
  bool randomizeOrder =
      postflopCombos > 10000 && preflopCombos <= 2 * MAX_LOOKUP_SIZE;
  ...
  for (;; ++enumPosition) {
    ...
    if (enumPosition >= enumEnd) {
      ...
      if (enumPosition >= enumEnd) break;
    }

    // Use a quasi-RNG to randomize the preflop enumeration order, while still
    // making sure every combo is evaluated once.
    uint64_t randomizedEnumPos =
        randomizeOrder ? urng(enumPosition) : enumPosition;
    ...
    for (unsigned i = 0; i < combinedRangeCount; ++i) {
      ...
      uint64_t remainder =
          randomizedEnumPos - quotient * mCombinedRanges[i].combos().size();
      randomizedEnumPos = quotient;

      const CombinedRange::Combo& combo =
          mCombinedRanges[i].combos()[(size_t)remainder];
      if (usedCardsMask & combo.cardMask) {
        ok = false;
        break;
      }
    ...
    if (!ok) {
      ...
      // Transform preflop into canonical form so that suit and player
      // isomoprhism can be detected.
      uint64_t boardCards = mBoardCards;
      uint64_t deadCards = mDeadCards;
      if (useLookup) {
        // Sort players based on their hand.
        std::sort(
            playerHands, playerHands + nplayers,
            [](const HandWithPlayerIdx& lhs, const HandWithPlayerIdx& rhs) {
        ...
        // Save original player indexes cause we eventually want the results for
        // the original order.
        ...
        for (unsigned j = 0; j < nplayers; ++j)
          usedCardsMask |= (1ull << playerHands[j].cards[0]) |
                           (1ull << playerHands[j].cards[1]);
        ...
        ++stats.uniquePreflopCombos;
        enumerateBoard(playerHands, nplayers, fixedBoard, usedCardsMask,
                       &stats);
      }
    }

    // TODO combine lookup results here so we don't need update so often
    if (stats.evalCount >= 10000 || stats.skippedPreflopCombos >= 10000 ||
        useLookup) {
      updateResults(stats, false);
      stats = BatchResults(nplayers);
      if (mStopped) break;
...
void EquityCalculator::enumerateBoard(const HandWithPlayerIdx* playerHands,
                                      unsigned nplayers, const Hand& board,
                                      uint64_t usedCardsMask,
                                      BatchResults* stats) {
  Hand hands[MAX_PLAYERS];
  for (unsigned i = 0; i < nplayers; ++i) hands[i] = Hand(playerHands[i].cards);
  ...
  // Initialize deck. This also determines the enumeration order. Iterating
  // ranks in descending order is ~5% faster for some reason. Could be better
  // branch prediction, because lower cards affect hand value less. It's
  // unlikely to be due to caching, because reversing the evaluator's rank
  // multipliers has no effect.
  unsigned deck[CARD_COUNT];
  unsigned ndeck = 0;
  for (unsigned c = CARD_COUNT; c-- > 0;) {
    if (!(usedCardsMask & (1ull << c))) deck[ndeck++] = c;
  }

  // Calculate the maximum card count for each suit that any player can have
  // after holecards and fixed board cards.
  unsigned suitCounts[SUIT_COUNT] = {};
  for (unsigned i = 0; i < nplayers; ++i) {
    if ((playerHands[i].cards[0] & 3) == (playerHands[i].cards[1] & 3)) {
      suitCounts[playerHands[i].cards[0] & 3] =
          std::max(2u, suitCounts[playerHands[i].cards[0] & 3]);
    } else {
      suitCounts[playerHands[i].cards[0] & 3] =
          std::max(1u, suitCounts[playerHands[i].cards[0] & 3]);
      suitCounts[playerHands[i].cards[1] & 3] =
          std::max(1u, suitCounts[playerHands[i].cards[1] & 3]);
    }
  }
  for (unsigned i = 0; i < SUIT_COUNT; ++i) suitCounts[i] += board.suitCount(i);

  enumerateBoardRec(hands, nplayers, stats, board, deck, ndeck, suitCounts,
                    remainingCards, 0, 1);
}

// Enumerates board cards recursively. Detects some isomorphic subtrees by
// looking at the number of cards for each suit. Suits that cannot create a
// flush anymore (called here "irrelevant suits") are handled at the same time,
// which gives roughly a speedup of 3x.
void EquityCalculator::enumerateBoardRec(const Hand* playerHands,
                                         unsigned nplayers, BatchResults* stats,
                                         const Hand& board, unsigned* deck,
                                         unsigned ndeck, unsigned* suitCounts,
                                         unsigned cardsLeft, unsigned start,
                                         unsigned weight) {
  // More efficient version for the innermost loop.
  if (cardsLeft == 1) {
    // Even simpler version for non-flush rivers.
    if (suitCounts[0] < 4 && suitCounts[1] < 4 && suitCounts[2] < 4 &&
        suitCounts[3] < 4) {
      for (unsigned i = start; i < ndeck;) {
        ...
        for (++i; i < ndeck && deck[i] >> 2 == rank; ++i) ++multiplier;

        evaluateHands<false>(playerHands, nplayers, newBoard, stats,
                             multiplier * weight);
      }
    } else {
      unsigned lastRank = ~0;
      for (unsigned i = start; i < ndeck; ++i) {
        ...
        if (suitCounts[deck[i] & 3] < 4) {
          ...
          // Since this is last card there's no need to do reorder deck cards;
          // we just count the irrelevant suits in current rank.
          for (unsigned j = i + 1; j < ndeck && deck[j] >> 2 == rank; ++j) {
            if (suitCounts[deck[j] & 3] < 4) ++multiplier;
          }
          lastRank = rank;
        }

        Hand newBoard = board + deck[i];
        evaluateHands(playerHands, nplayers, newBoard, stats,
                      multiplier * weight);
      }
    }
    return;
  ...
  for (unsigned i = start; i < ndeck; ++i) {
    ...
    if (suitCounts[suit] + cardsLeft < 5) {
      ...
      // Go through all the cards with same rank (they're always consecutive)
      // and count the irrelevat suits.
      for (unsigned j = i + 1; j < ndeck && deck[j] >> 2 == rank; ++j) {
        unsigned suit2 = deck[j] & 3;
        if (suitCounts[suit2] + cardsLeft < 5) {
          // Move all the irrelevant suits before other suits so they don't get
          // used again.
      ...
      // When there are multiple cards with irrelevant suits we have to choose
      // how many of them to use, and the number of isomorphic subtrees depends
      // on it.
      for (unsigned repeats = 1;
           repeats <= std::min(irrelevantCount, cardsLeft); ++repeats) {
        static const unsigned BINOM_COEFF[5][5] = {
            {0}, {0, 1}, {1, 2, 1}, {1, 3, 3, 1}, {1, 4, 6, 4, 1}};
        ...
        if (repeats == cardsLeft)
          ...
          enumerateBoardRec(playerHands, nplayers, stats, newBoard, deck, ndeck,
                            suitCounts, cardsLeft - repeats,
                            i + irrelevantCount, newWeight);
      }
      ...
      ++suitCounts[suit];
      enumerateBoardRec(playerHands, nplayers, stats, newBoard, deck, ndeck,
                        suitCounts, cardsLeft - 1, i + 1, weight);
      --suitCounts[suit];
    }
  }
}
...
bool EquityCalculator::lookupResults(uint64_t preflopId,
                                     BatchResults& results) {
  if (!mDeadCards && !mBoardCards &&
      lookupPrecalculatedResults(preflopId, results))
    return true;

  std::lock_guard<std::mutex> lock(mMutex);
  auto it = mLookup.find(preflopId);
  if (it != mLookup.end()) results = it->second;
  return it != mLookup.end();
}

// Lookup precalculated results.
bool EquityCalculator::lookupPrecalculatedResults(uint64_t preflopId,
                                                  BatchResults& results) const {
  struct Cmp {
    // Need two overloads because of the asymmetry.
    bool operator()(uint32_t preflopId, uint64_t a) {
      return preflopId < (a & 0x3fffff);
    }
    bool operator()(uint64_t a, uint32_t preflopId) {
      return (a & 0x1fffff) < preflopId;
    }
  };

  // Binary search.
  auto it = std::lower_bound(PRECALCULATED_2PLAYER_RESULTS.begin(),
                             PRECALCULATED_2PLAYER_RESULTS.end(),
                             (uint32_t)preflopId, Cmp());
  if (it == PRECALCULATED_2PLAYER_RESULTS.end() ||
      (*it & 0x3fffff) != preflopId)
    return false;

  // Unpack results.
  results.winsByPlayerMask[1] = (*it >> 22) & 0x1fffff;
  results.winsByPlayerMask[3] = (*it >> 43) & 0x1fffff;
  results.winsByPlayerMask[2] =
      1712304 - results.winsByPlayerMask[1] - results.winsByPlayerMask[3];

  return true;
}

// Store results for one preflop in the lookup table.
void EquityCalculator::storeResults(uint64_t preflopId,
                                    const BatchResults& results) {
  std::lock_guard<std::mutex> lock(mMutex);  // TODO read-write lock
  mLookup.emplace(preflopId, results);
  // Make sure the hash map doesn't eat all memory. Not a great way of doing it
  // but the lookup table is quite useless with that many preflop combos anyway.
  if (mLookup.size() >= MAX_LOOKUP_SIZE) mLookup.clear();
}

// Transforms suits in such way that suit isomorphism can be easily detected.
// Goes through all the holecards, board cards and dead cards. First encountered
// suit is mapped to "virtual" suit 0, second suit maps to 1 and so on.
unsigned EquityCalculator::transformSuits(HandWithPlayerIdx* playerHands,
                                          unsigned nplayers,
                                          uint64_t* boardCards,
                                          uint64_t* deadCards) {
  ...
  for (unsigned i = 0; i < CARD_COUNT; ++i) {
    if ((*boardCards >> i) & 1) {
      ...
      if (transform[suit] == ~0u) transform[suit] = suitCount++;
  ...
  *deadCards = newDeadCards;
  ...
  // Holecards need to be handled after any fixed cards, because the lookup is
  // only based on them.
  for (unsigned i = 0; i < nplayers; ++i) {
...
uint64_t EquityCalculator::calculateUniquePreflopId(
    const HandWithPlayerIdx* playerHands, unsigned nplayers) {
  uint64_t preflopId = 0;
  // Basically we just map the preflop to a number in base 1327, where each
  // digit represents a hand.
  for (unsigned i = 0; i < nplayers; ++i) {
    preflopId *= (CARD_COUNT * (CARD_COUNT - 1) >> 1) + 1;  // 1327
    auto h = playerHands[i].cards;
    if (h[0] < h[1]) std::swap(h[0], h[1]);
    preflopId +=
        (h[0] * (h[0] - 1) >> 1) + h[1] + 1;  // map a hand to range [0, 1326]
  }
  return preflopId;
}
...
EquityCalculator::removeInvalidCombos(const std::vector<CardRange>& handRanges,
  ...
  for (auto& hr : handRanges) {
    ...
    for (auto& h : hr.combinations()) {
      ...
      if (!(reservedCards & handMask)) result.back().push_back(h);
    }
  }
  return result;
}
...
std::pair<uint64_t, uint64_t> EquityCalculator::reserveBatch(
    uint64_t batchCount) {
  std::lock_guard<std::mutex> lock(mMutex);
  ...
  uint64_t end =
      std::min<uint64_t>(totalBatchCount, mEnumPosition + batchCount);
  mEnumPosition = end;

  return {start, end};
}

// Number of different preflops with given hand ranges, assuming no conflicts
// between players' hands.
...
// Calculates size of the postflop tree, i.e. n choose k, where n is remaining
// deck size and k is number of undealt board cards.
uint64_t EquityCalculator::getPostflopCombinationCount() {
  ...
  for (unsigned i = 0; i < boardCardsRemaining; ++i) postflopCombos /= i + 1;
  return postflopCombos;
}

// Results aggregation for both enumeration and monte carlo.
void EquityCalculator::updateResults(const BatchResults& stats,
                                     bool threadFinished) {
  ...
  if (mResults.time + dt >= mTimeLimit ||
      mResults.hands + mResults.intervalHands >= mHandLimit)
    mStopped = true;
  ...
  if (dt >= mUpdateInterval || mResults.finished) {
    ...
    mResults.intervalSpeed =
        mResults.intervalHands / (mResults.intervalTime + 1e-9);
    mResults.speed = mResults.hands / (mResults.time + 1e-9);
    mResults.intervalHands = 0;
    mResults.stdev =
        std::sqrt(1e-9 + mBatchSumSqr - mBatchSum * mBatchSum / mBatchCount) /
        mBatchCount;
    ...
    if (mResults.enumerateAll) {
      ...
      double estimatedHands =
          std::pow(mResults.stdev / mStdevTarget, 2) * mResults.hands;
      mResults.progress = mResults.hands / estimatedHands;
    }
    mResults.preflopCombos = getPreflopCombinationCount();

    if (!mResults.enumerateAll &&
        mResults.stdev < mStdevTarget)  // TODO use max stdev of any player
      mStopped = true;

    for (unsigned i = 0; i < mResults.players; ++i)
      mResults.equity[i] =
          (mResults.wins[i] + mResults.ties[i]) / (mResults.hands + 1e-9);

    mUpdateResults = mResults;

    if (mCallback) mCallback(mResults);
    ...
    for (unsigned j = 0; j < mResults.players; ++j) {
      if (i & (1 << j)) {
        if (winnerCount == 1) {
          ...
          if (batch.playerIds[j] == 0) batchEquity += batch.winsByPlayerMask[i];
        } else {
          mResults.ties[batch.playerIds[j]] +=
              batch.winsByPlayerMask[i] / (double)winnerCount;
...
void EquityCalculator::outputLookupTable() const {
  ...
  std::sort(a.begin(), a.end(),
            [](const std::array<unsigned, 3>& lhs,
               const std::array<unsigned, 3>& rhs) { return lhs[0] < rhs[0]; });

  std::cout << std::hex;
  for (size_t i = 0; i < a.size(); ++i) {
    if (i % 6 == 0) std::cout << std::endl;
    std::cout << " 0x"
              << ((uint64_t)a[i][0] | (uint64_t)a[i][1] << 22 |
                  (uint64_t)a[i][2] << 43)
              << ",";
  }
  std::cout << std::endl;
