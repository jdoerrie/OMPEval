...
#define OMP_SSE2 1
// MSVC has no way to distuingish between SSE2/SSE4, so we only enable SSE4
// there with AVX.
...
static inline void* alignedNew(size_t size, unsigned alignment) {
  ...
  if (alignment < OMP_ALIGNOF(void*)) alignment = OMP_ALIGNOF(void*);
  ...
  // Save the original pointer before the start of the aligned block. (Needed
  // for dealloc.)
...
// Custom allocator that guarantees correct aligment inside standard containers
// for objects that have alignment greater than std::max_align_t.
template <class T>
class AlignedAllocator {
 public:
  ...
  void deallocate(T* p, size_t n) { alignedDelete(p); }
  ...
  // The following stuff should not be needed but some compilers aren't fully
  // compatible.
...
// Specializes std::allocator for TYPE and pair<T,TYPE>. This is for convenience
// so that it's not necessary to specify a custom allocator as a template
// argument for STL containers. It's a bit of a hack, because the specialization
// isn't fully conformant (rebind isn't symmetric). allocator_traits is
// specialized to fix some issues caused by that. On x64 the allocator isn't
// needed because default allocation is typically 16-byte aligned.
#if OMP_SSE2 && !OMP_X64
#define OMP_ALIGNED_STD_ALLOCATOR(TYPE)                                   \
  ...
  class allocator<TYPE> : public omp::AlignedAllocator<TYPE> {            \
   public:                                                                \
    ...
    template <class U>                                                    \
    allocator(const omp::AlignedAllocator<U>&) {}                         \
  };                                                                      \
  template <typename T>                                                   \
  class allocator<pair<T, TYPE>>                                          \
      : public omp::AlignedAllocator<pair<T, TYPE>> {                     \
   public:                                                                \
    allocator() = default;                                                \
    template <class U>                                                    \
    allocator(const allocator<U>&) {}                                     \
  ...
  class allocator_traits<allocator<TYPE>>                                 \
      : public allocator_traits<omp::AlignedAllocator<TYPE>> {};          \
  template <typename T>                                                   \
