...
// Structure that combines the data from multiple cards so that hand strength
// can be evaluated efficiently. It is essential when combining hands that
// exactly one of them was initialized using Hand::empty().
struct Hand {
  // Default constructor. Leaves the struct uninitialized for performance
  // reasons.
  Hand() {
#if OMP_SSE2
  ...
  // Create a Hand from a card. CardIdx is an integer between 0 and 51, so that
  // CARD = 4 * RANK + SUIT, where rank ranges from 0 (deuce) to 12 (ace) and
  // suit is from 0 (spade) to 3 (diamond).
  ...
  static Hand empty() {
    // Initializes suit counters to 3 so that the flush check bits gets set by
    // the 5th suited card.
    return EMPTY;
  }
  ...
  bool hasFlush() const {
    // Hand has a 4-bit counter for each suit. They start at 3 so the 4th bit
    // gets set when there is 5 or more cards of that suit. We can check for
    // flush by simply masking out the flush check bits.
  ...
  uint16_t flushKey() const {
    // Get the index of the flush check bit and use it to get the card mask for
    // that suit.
...
// Bits 0-31: Key to non-flush lookup table (linear combination of the rank
// constants). Bits 32-35: Card counter. Bits 48-63: Suit counters. Bits 64-128:
// Bit mask for all cards (suits are in 16-bit groups).
#if OMP_SSE2
  __m128i mData;
#else
  uint64_t mKey;
...
// Some 32-bit compilers don't align __m128i correctly inside containers, which
// causes segfault. We need a custom allocator to fix the alignment.
