...
CombinedRange::CombinedRange() : mPlayerCount(0), mSize(0) {}

CombinedRange::CombinedRange(
    unsigned playerIdx, const std::vector<std::array<uint8_t, 2>>& holeCards) {
  mPlayerCount = 1;
...
CombinedRange CombinedRange::join(const CombinedRange& range2) const {
  ...
  std::copy(mPlayers.begin(), mPlayers.begin() + mPlayerCount,
            newRange.mPlayers.begin());
  std::copy(range2.mPlayers.begin(),
            range2.mPlayers.begin() + range2.mPlayerCount,
            newRange.mPlayers.begin() + mPlayerCount);

  for (const Combo& c1 : mCombos) {
    for (const Combo& c2 : range2.mCombos) {
      if (c1.cardMask & c2.cardMask) continue;
      Combo c;
      c.cardMask = c1.cardMask | c2.cardMask;
      std::copy(std::begin(c1.holeCards),
                std::begin(c1.holeCards) + mPlayerCount,
                std::begin(c.holeCards));
      std::copy(std::begin(c2.holeCards),
                std::begin(c2.holeCards) + range2.mPlayerCount,
                std::begin(c.holeCards) + mPlayerCount);
  ...
  for (;;) {
    ...
    for (unsigned i = 0; i < combinedRanges.size(); ++i) {
      for (unsigned j = 0; j < i; ++j) {
        uint64_t newSize =
            combinedRanges[i].estimateJoinSize(combinedRanges[j]);
        if (newSize < bestSize) besti = i, bestj = j, bestSize = newSize;
      }
    }
