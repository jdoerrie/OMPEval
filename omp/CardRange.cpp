...
CardRange::CardRange(const std::string& text) {
  ...
  std::string s;
  for (char c : text) {
    if (std::isgraph(c, loc)) s.push_back(std::tolower(c, loc));
  }
...
CardRange::CardRange(const char* text) : CardRange(std::string(text)) {}

// Construct from vctor.
CardRange::CardRange(const std::vector<std::array<uint8_t, 2>>& combos) {
  for (auto& combo : combos) addCombo(combo[0], combo[1]);
  removeDuplicates();
}
...
bool CardRange::parseHand(const char*& p) {
  ...
  if (!parseRank(p, r1)) return false;
...
void CardRange::addCombos(unsigned rank1, unsigned rank2, bool suited,
                          bool offsuited) {
  ...
  if (offsuited) {
    for (unsigned suit1 = 0; suit1 < 4; ++suit1)
      for (int suit2 = suit1 + 1; suit2 < 4; ++suit2) {
        ...
        if (rank1 != rank2) addCombo(4 * rank1 + suit2, 4 * rank2 + suit1);
      }
  }
}

// Add range of hands defined by the "+" suffix.
void CardRange::addCombosPlus(unsigned rank1, unsigned rank2, bool suited,
                              bool offsuited) {
  if (rank1 == rank2) {
    for (unsigned r = rank1; r < 13; ++r) addCombos(r, r, suited, offsuited);
  } else {
...
void CardRange::addAll() {
  for (unsigned c1 = 0; c1 < CARD_COUNT; ++c1)
    for (unsigned c2 = 0; c2 < c1; ++c2) addCombo(c1, c2);
}
...
void CardRange::removeDuplicates() {
  std::sort(
      mCombinations.begin(), mCombinations.end(),
      [](const std::array<uint8_t, 2>& lhs, const std::array<uint8_t, 2>& rhs) {
        if (lhs[0] >> 2 != rhs[0] >> 2) return lhs[0] >> 2 < rhs[0] >> 2;
        if (lhs[1] >> 2 != rhs[1] >> 2) return lhs[1] >> 2 < rhs[1] >> 2;
        if ((lhs[0] & 3) != (rhs[0] & 3)) return (lhs[0] & 3) < (rhs[0] & 3);
