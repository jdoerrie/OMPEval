...
class XoroShiro128Plus {
 public:
  ...
  XoroShiro128Plus(uint64_t seed) { mState[0] = ~(mState[1] = seed); }
...
// Generates non-repeating pseudo random numbers in specified range using a
// linear congruential generator.
class UniqueRng64 {
 public:
  UniqueRng64(uint64_t range) : mRange(range) {
 ...
 private:
  // Probably not ideal constants in most cases but guarantee a full period for
  // any LCG.
...
// Simple and fast uniform int distribution for small ranges. Has a bias similar
// to the classic modulo method, but it's good enough for most poker
// simulations.
...
class FastUniformIntDistribution {
 public:
  ...
  FastUniformIntDistribution() { init(0, 1); }

  FastUniformIntDistribution(T min, T max) { init(min, max); }
  ...
  template <class TRng>
  T operator()(TRng& rng) {
    static_assert(sizeof(typename TRng::result_type) == sizeof(uint64_t),
                  "64-bit RNG required.");
...
// A bit slower distribution without bias (still faster than
...
class FastUniformIntDistribution2 {
 public:
  ...
  FastUniformIntDistribution2() { init(0, 1); }

  FastUniformIntDistribution2(T min, T max) { init(min, max); }
  ...
  T operator()(TRng& rng) {
    static_assert(sizeof(typename TRng::result_type) == sizeof(uint64_t),
                  "64-bit RNG required.");
    if (mBufferUsesLeft == 0) getBits(rng);
 ...
 private:
  ...
  void initializeConstants() {
    ...
    if ((diffPow & 0xffffffffull) == 0 &&
        ((diffPow >> 32) * mDiff == 0x100000000ull)) {
      ++mMaxBufferUses;
      diffPow *= mDiff;
    }
    mMaxBufferVal = diffPow - 1;

    while (((mMask >> 1) & mMaxBufferVal) == mMaxBufferVal) mMask >>= 1;
  }
