...
class CardRange {
 public:
  ...
  // random : all hands
  // Spaces and non-matching characters in the end are ignored. The expressions
  ...
  // Returns a list of card combinations belonging to this range. Guarantees
  // that there are no duplicates. Cards in each combo are ordered so that the
  // bigger rank is always first. The whole vector is sorted in the following
  // order: 1) rank of first card 2) rank of second card 3) suit of first card
  // 4) suit of second card
 ...
 private:
  ...
  void addCombosPlus(unsigned rank1, unsigned rank2, bool suited,
                     bool offsuited);
