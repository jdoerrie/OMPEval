...
class HandEvaluator {
 public:
  ...
  // Returns the rank of a hand as a 16-bit integer. Higher value is better. Can
  // also rank hands with less than 5 cards. A missing card is considered the
  // worst kicker, e.g. K < KQJT8 < A < AK < KKAQJ < AA < AA2 < AA4 < AA432.
  // Hand category can be extracted by dividing the value by 4096. 1=highcard,
  // 2=pair, etc.
 ...
 private:
  ...
  // Determines in how many rows the original lookup table is divided (2^shift).
  ...
  // Minimum number of cards required for evaluating a hand. Can be set to
  // higher value to decrease lookup table size (requires hash recalculation).
  ...
  static uint32_t
      PERF_HASH_ROW_OFFSETS[8191 + RECALCULATE_PERF_HASH_OFFSETS * 100000];
};
